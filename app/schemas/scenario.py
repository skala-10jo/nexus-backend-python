...
class StepInfo(BaseModel):
    """Step information for structured conversation flow."""
    name: str = Field(..., description="Step identifier (e.g., ice_breaking, agenda1, wrap_up)")
    title: str = Field(..., description="Step title for display (e.g., 인사, 메인 아젠다)")
    guide: str = Field(..., description="Guide text explaining what to do in this step")
    terminology: List[str] = Field(default=[], description="Key terms/expressions for this step")
...
class GenerateFromProjectsRequest(BaseModel):
    ...
    userRequest: Optional[str] = Field(default=None, alias="userRequest", description="User's specific request for scenario generation (e.g., situation, focus area)")
...
class ManualCreateRequest(BaseModel):
    ...
    steps: Optional[List[StepInfo]] = Field(default=None, description="Structured conversation steps")
    language: str = Field(default="en", description="Target language: en, ko, zh, ja")
    difficulty: str = Field(default="intermediate", description="Difficulty level: beginner, intermediate, advanced")
