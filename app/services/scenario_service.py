...
class ScenarioService:
    ...
    async def generate_from_projects(
        ...
        """
        logger.info(f"🚀 Starting scenario generation: user={user_id}, projects={len(project_ids)}, schedules={len(schedule_ids)}, documents={len(document_ids)}")

        # 1. Fetch context from Java backend
        context = await self._fetch_context(project_ids, schedule_ids, document_ids, jwt_token)
        logger.info(f"📝 Context fetched: {len(context)} characters")

        # 2. Generate scenarios with GPT-4o
        scenarios_data = await self._generate_with_gpt(context, language, difficulty, count)
        logger.info(f"🤖 GPT-4o generated {len(scenarios_data)} scenarios")
        ...
        for idx, scenario_data in enumerate(scenarios_data):
            try:
                ...
                logger.info(f"💾 Saved scenario {idx + 1}/{len(scenarios_data)}: {scenario.title}")

            except Exception as e:
                logger.error(f"❌ Failed to save scenario {idx + 1}: {str(e)}")
                continue

        db.commit()
        logger.info(f"✅ Successfully saved {len(saved_scenarios)}/{len(scenarios_data)} scenarios")
    ...
    async def _fetch_context(
            ...
            for pid in project_ids:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"⚠️  Failed to fetch project {pid}: status {response.status_code}")
                except Exception as e:
                    logger.warning(f"⚠️  Failed to fetch project {pid}: {str(e)}")
            ...
            for sid in schedule_ids:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"✅ Fetched schedule: {schedule.get('title', '')}")
                    else:
                        logger.warning(f"⚠️  Failed to fetch schedule {sid}: status {response.status_code}")
                except Exception as e:
                    ...
                    if response.status_code == 200:
                        ...
                        if contents:
                            ...
                            if text_parts:
                                ...
                                logger.info(f"✅ Fetched document: {document.get('originalFilename', '')} ({len(contents)} pages)")
                        else:
        ...
        if not context_parts:
            logger.warning("⚠️  No context fetched, using general business scenarios")
